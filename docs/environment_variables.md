...
| --- | --- | --- | --- |
| `ORCHEO_CHECKPOINT_BACKEND` | `postgres` | `sqlite` or `postgres` | Selects the checkpoint persistence backend consumed by `config/loader.py`. |
...
| `ORCHEO_GRAPH_STORE_BACKEND` | `postgres` | `sqlite` or `postgres` | Selects the LangGraph store backend used for graph memory/state storage (`config/loader.py`, `persistence.py`). |
...
| `ORCHEO_REPOSITORY_BACKEND` | `postgres` | `sqlite`, `postgres`, or `inmemory` | Chooses the workflow repository implementation (`config/loader.py`). |
...
| `ORCHEO_CHATKIT_BACKEND` | `postgres` | `sqlite` or `postgres` | Selects the ChatKit persistence backend used by `chatkit/server.py`. |
