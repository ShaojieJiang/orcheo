...
This guide covers manual installation and configuration of Orcheo for users who prefer direct control over their setup, as well as information for developers contributing to the project.
...
For interactive CLI usage, authenticate via browser-based OAuth using the same identity provider as Canvas:
...
For detailed configuration, security best practices, and troubleshooting, see [authentication_guide.md](authentication_guide.md).
...
**Note:** This configuration requires the Orcheo development backend to be running locally.
...
This section is for developers contributing to the Orcheo project.
...
See [deployment.md](deployment.md) for Docker Compose and managed PostgreSQL deployment recipes.
...
Learn how to extend Orcheo with your own nodes, tool integrations, and workflow helpers in [custom_nodes_and_tools.md](custom_nodes_and_tools.md).

## Workflow Repository Configuration

The FastAPI backend supports pluggable workflow repositories so local development can persist state without depending on Postgres. By default the app uses a SQLite database located at `~/.orcheo/workflows.sqlite`. Adjust the following environment variables to switch behaviour:
...
Refer to `.env.example` for sample values and to [deployment.md](deployment.md) for deployment-specific guidance.
