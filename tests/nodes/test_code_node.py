...
async def test_basic_code_execution():
    ...
    assert output["node_outputs"] == {"python_node": 3}
...
async def test_code_with_state():
    ...
    node = PythonCode(
        name="python_node",
        code="""
x = state["node_outputs"][-1]["x"]
y = state["node_outputs"][-1]["y"]
a = x * 2
b = y + 1
result = a + b
return result
""",
    ...
    output = await node(state, None)
    assert output["node_outputs"] == {"python_node": 8}
...
""",
    ...
    output = await node(state, None)
    assert output["node_outputs"] == {"python_node": 3.141592653589793}
