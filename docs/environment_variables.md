...
| `ORCHEO_POSTGRES_DSN` | _none_ | PostgreSQL DSN (e.g. `postgresql://user:pass@host:port/db`) | Connection string required when `ORCHEO_CHECKPOINT_BACKEND=postgres` or `ORCHEO_REPOSITORY_BACKEND=postgres` (see [config/loader.py](../src/orcheo/config/loader.py)). |
