...
- [ ] Task 0.1: Vendor/fork `agentensor` into `packages/agentensor` with compatible `pyproject`/uv metadata
...
- [ ] Task 1.3: Persist run metadata (config, tags, callbacks) and emit observability signals
...
- [ ] Task 2.2: Add API/CLI entry to trigger evaluation runs and stream progress
...
- [ ] Task 3.2: Persist checkpoints and best-performing configs in a dedicated DB table keyed by `workflow_id` with versioned records; expose download/reuse endpoints
  - Dependencies: Task 3.1
- [ ] Task 3.3: Add safeguards (concurrency limits, timeout caps) and end-to-end tests for training mode
  - Dependencies: Task 3.1
