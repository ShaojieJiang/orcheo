...
- **Platform Adapters**
  ...
  - **QQ Gateway Adapter**
    - Exchanges `AppID` and `AppSecret` for an access token.
    - Calls `GET /gateway`, opens WSS connection, maintains heartbeat, persists `sessionId` plus sequence, and converts QQ events into normalized listener events.
...
4. The adapter exchanges `AppID` and `AppSecret` for an access token.
5. The adapter calls the QQ Bot `/gateway` API, opens the returned WebSocket URL, starts heartbeats, and begins receiving events.
...
- **Template validation**: manually validate one Canvas template per platform with `listener -> AgentNode -> supported reply node` wiring and one Canvas template with all three listeners feeding a shared `AgentNode`.
- **Manual QA checklist**: private-network deployment, token rotation, workflow pause/resume, two independent Telegram bots, one Discord bot in DM and guild scenarios, and two independent QQ bots using different AppIDs.
...
- The exact QQ event envelope and permission model should be validated against the current Tencent QQ Bot API v2 documentation during implementation, even though the OpenClaw plugin confirms that the gateway-session approach is viable.
