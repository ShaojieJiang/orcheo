...
| `ORCHEO_CHATKIT_STORAGE_PATH` | `~/.orcheo/chatkit` | Directory path | Filesystem root for ChatKit attachments ([config/loader.py](../src/orcheo/config/loader.py)). |
| `ORCHEO_CHATKIT_CDN_BASE_URL` | `https://cdn.platform.openai.com/` | HTTP(S) URL | Overrides the upstream CDN base used by the ChatKit asset proxy routes ([chatkit_asset_proxy.py](../apps/backend/src/orcheo_backend/app/chatkit_asset_proxy.py)). |
